...
/**
 * Represents a message sent through the MessageChannel between the client and server.
 *
 * @param type The type of the message, which can be one of "BRIDGE_REQUEST", "BRIDGE_RESPONSE", or "EVENT".
 * @param payload The payload of the message, varies on the type.
 */
export type PortMessage =
    | { type: "BRIDGE_REQUEST"; payload: BridgeRequest }
    | { type: "BRIDGE_RESPONSE"; payload: BridgeResponse }
...
export interface BridgeResponse {
    /**
     * The unique identifier of the original request that this response corresponds to.
     */
    id: string;
    /**
     * A boolean indicating whether the server successfully processed the request.
     */
    success: boolean;
    /**
     * If `success` is true, this field contains the data returned by the server method. Otherwise, it may be undefined.
     */
    data?: unknown;
    /**
     * If `success` is false, this field contains an error message describing what went wrong. Otherwise, it may be undefined.
     */
    error?: string;
}
...
export interface BridgeEvent {
    ...
    eventName: string;
    /**
     * The data payload associated with the event. This can be of any type and is defined by the server when emitting the event.
     */
    data: unknown;
}
