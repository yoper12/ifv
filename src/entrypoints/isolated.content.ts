    ...
    main() {
        ...
        async function runSync(
            ...
            if (lifecycle.document_start) {
                await syncPatches(
                    patches,
                    { world: "ISOLATED", runAt: "document_start" },
                    trigger,
                    changedPatches,
                );
            }
            if (lifecycle.document_end) {
                await syncPatches(
                    patches,
                    { world: "ISOLATED", runAt: "document_end" },
                    trigger,
                    changedPatches,
                );
            }
        ...
        if (document.readyState === "loading") {
            window.addEventListener(
                "DOMContentLoaded",
                () => {
                    syncPatches(patches, { world: "ISOLATED", runAt: "document_end" }, "INITIAL");
        ...
        if (document.readyState === "complete") {
            syncPatches(patches, { world: "ISOLATED", runAt: "document_idle" }, "INITIAL");
