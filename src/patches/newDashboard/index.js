...
const applyIcons = () => {
    for (const [tileTitle, fileName] of icons) {
        ...
        container.parentElement?.parentElement?.addEventListener("click", (e) => {
...
const createToolbar = async () => {
    const getContainer = () =>
        document.querySelector(".content-container > .tile-container > .tile-subcontainer");
    ...
    const getLuckyNumber = () =>
        document.querySelector(".lucky-number__circle.lucky-number__number > span")?.innerText;
    waitForRender(getLuckyNumber).then(
        () => (element.querySelector("div:first-of-type > span").innerText = getLuckyNumber()),
    );
    ...
    waitForRender(getAmountOfMessages).then(
        () => (element.querySelector("div:last-of-type > span").innerText = getAmountOfMessages()),
    );
...
window.appendModule({
    ...
    onlyOnReloads: false,
    isLoaded: () => document.querySelector(".plan-zajec") && !document.querySelector(".spinner"),
});
